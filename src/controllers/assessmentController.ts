...
function evaluateStepResult(step: number, percentage: number) {
    // returns { levelAchieved, certification, canProceed, failureNoRetake }
    if (step === 1) {
        if (percentage < 25) return { levelAchieved: null, certification: 'Fail', canProceed: false, failureNoRetake: true };
        if (percentage < 50) return { levelAchieved: 'A1', certification: 'A1 certified', canProceed: false, failureNoRetake: false };
        if (percentage < 75) return { levelAchieved: 'A2', certification: 'A2 certified', canProceed: false, failureNoRetake: false };
        return { levelAchieved: 'A2', certification: 'A2 certified', canProceed: true, failureNoRetake: false };
    }
    if (step === 2) {
        if (percentage < 25) return { levelAchieved: 'A2', certification: 'Remain at A2', canProceed: false, failureNoRetake: false };
        if (percentage < 50) return { levelAchieved: 'B1', certification: 'B1 certified', canProceed: false, failureNoRetake: false };
        if (percentage < 75) return { levelAchieved: 'B2', certification: 'B2 certified', canProceed: false, failureNoRetake: false };
        return { levelAchieved: 'B2', certification: 'B2 certified', canProceed: true, failureNoRetake: false };
    }
    if (step === 3) {
        if (percentage < 25) return { levelAchieved: 'B2', certification: 'Remain at B2', canProceed: false, failureNoRetake: false };
        if (percentage < 50) return { levelAchieved: 'C1', certification: 'C1 certified', canProceed: false, failureNoRetake: false };
 ...
 */
export async function getQuestionsForStep(req: Request, res: Response) {
...
export async function startAssessment(req: Request, res: Response) {
    try {
        ...
        if (!userIdStr) return res.status(401).json({ success: false, message: 'Unauthorized' });

        const step = Number(req.body.step);
        if (![1, 2, 3].includes(step)) return res.status(400).json({ success: false, message: 'Invalid step' });

        // check permission: user must be allowed to take this step
        const user = await SchoolUser.findById(userIdStr);
...
export async function getCurrentSession(req: Request, res: Response) {
    ...
    const session = await AssessmentSession.findOne({ userId: userIdStr, status: 'in_progress' }).lean();
    if (!session) return res.json({ success: true, data: null });

    // populate questions details for UI if needed (hide correctAnswerIndex)
    const questions = await Question.find({ _id: { $in: session.questions } }).select('questionText options competency level').lean();
...
export async function submitAnswer(req: Request, res: Response) {
    try {
        ...
        if (session.userId.toString() !== userIdStr) return res.status(403).json({ success: false, message: 'Forbidden' });
 ...
 */
export async function canTakeAssessment(req: Request, res: Response) {
    try {
        const userIdStr = (req as any).user?.id;
        if (!userIdStr) return res.status(401).json({ success: false, message: 'Unauthorized' });
